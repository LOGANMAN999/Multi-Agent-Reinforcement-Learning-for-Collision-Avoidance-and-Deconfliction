...
class AStarGlobalLocalController:
    ...
    def __call__(self, env) -> np.ndarray:
        ...
        for i in range(N):
            ...
            if need_replan:
                ...
                for scale in (1.0, 0.7):
                    ...
                    self._plan_paths_for_all_agents(env, skip_smoothing=True, inflate_scale=scale)
                    path = self._paths_world.get(i, [])
                    if path is None or len(path) == 0:
                        self._plan_paths_for_all_agents(env, skip_smoothing=False, inflate_scale=scale)
                        path = self._paths_world.get(i, [])
            ...
            while wp_idx < len(path) - 1:
                if np.linalg.norm(pos_i - path[wp_idx]) < tol:
                    ...
                    if self._segment_is_free(pos_i, path[wp_idx + 1], env, n_samples=n_samples):
                        wp_idx += 1
                    else:
                        break
                else:
        ...
        active = np.linalg.norm(positions - goals, axis=1) > env.goal_tolerance
